...
(function () {
  ...
  function getSession() {
    try {
      var session = JSON.parse(localStorage.getItem(AUTH_KEY) || "null");
  ...
  function setSession(payload) {
    localStorage.setItem(AUTH_KEY, JSON.stringify(Object.assign({}, payload, {
  ...
  function initLogin() {
    ...
    if (!form) return;
    if (getSession()) { location.href = "/admin/dashboard"; return; }
    form.addEventListener("submit", function (event) {
      ...
      if (username === "nawal" && password === "Areen12@n") {
        setSession({ isLoggedIn: true, username: username });
  ...
  function initDashboard() {
    ...
    if (logout) logout.addEventListener("click", function () { clearSession(); location.href = "/admin"; });
